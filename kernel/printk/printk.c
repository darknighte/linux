...
static lcb_t __lcb =
	...
	LOGBUFF_LOG_VERSION, LOGBUFF_CB_PAD_LENGTH, sizeof(lcb_t),
...
static int logbuf_has_space(u32 msg_size, bool empty)
{
	...
	if (lcb->log_next_idx > lcb->log_first_idx || empty)
		free = max(lcb->log_buf_len - lcb->log_next_idx, lcb->log_first_idx);
	else
		     ...
		     const char *text, u16 text_len)
{
	...
	if (lcb->log_next_idx + size + sizeof(struct printk_log) > lcb->log_buf_len) {
		/*
		 * This message + an additional empty header does not fit
		 * at the end of the buffer. Add an empty header with len == 0
		 * to signify a wrap around.
		 */
		memset(lcb->log_buf + lcb->log_next_idx, 0, sizeof(struct printk_log));
		lcb->log_next_idx = 0;
	}
...
phys_addr_t lcb_phy_addr __initdata = 0;

/* save requested lcb_phy_addr since it's too early to process it */
static int __init lcb_phy_addr_setup(char *str)
{
	lcb_phy_addr = memparse(str, &str);

	return 0;
}

early_param("lcb_phy_addr", lcb_phy_addr_setup);
 ...
 * expects.
 */
static bool is_valid_external_cb(const lcb_t const *new_lcb)
{
	/* Validate the new control buffer */
	return (new_lcb &&
		new_lcb->log_version == LOGBUFF_LOG_VERSION &&
		new_lcb->lcb_magic == LOGBUFF_MAGIC &&
		new_lcb->lcb_pad_len == LOGBUFF_CB_PAD_LENGTH &&
		new_lcb->lcb_size == sizeof(lcb_t) &&
		new_lcb->printk_log_size == sizeof(struct printk_log) &&
		new_lcb->lcb_size <= new_lcb->lcb_pad_len &&
 ...
 * Memory Reservation
 * ------------------
 * This feature expects the bootloader to reserve the memory using a separate
 * mechanism.  As an alternative, I considered adding memblock_reserve()
 * calls in setup_log_buf(), but was concerned about a narrow window of
 * opportunity for the kernel to claim or to modify that memory.
 *
 */
void __init setup_ext_logbuff(void)
{
	...
	pr_debug
	    ("log : Replacing existing log control block @ 0x%p of %lu bytes "
	     "with an external control block @ 0x%p of %d out of %d " "bytes\n",
	     lcb, sizeof(lcb_t), new_lcb, new_lcb->lcb_size,
	     new_lcb->lcb_pad_len);
	...
	if (lcb->log_buf_len != new_lcb->log_buf_len)
